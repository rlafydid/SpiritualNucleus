Shader "ORII/UI/FX_UI_Combine_Alpha"
{
    Properties
    {
        //[Header(Baisc Module)]
        ...
        [Toggle(ENABLE_Alpha)]_UseChannel ("Use Alpha Channel", Float) = 0
        _UseAlpha ("MainTex Alpha Channel", Vector) = (0, 0, 0, 0)
        ...
        _Lux ("Mask GLOW", Range(0, 10)) = 0
        _Cull ("Culling Switch", Float) = 0
        _ZTest ("ZTest Switch", Float) = 4
        _ZWrite ("ZWrite Switch", Float) = 0
    ...
    SubShader
    {
        ...
        {
            //Pass名字
            Name "Particle_Forward_CombineAlpha"
            Tags
            {
            ...
            #pragma shader_feature_local ENABLE_Alpha
            TEXTURE2D(_MainTex);
            ...
            half4 _TintColor;
            half4 _UseAlpha;
            half4 _ClipRange0 = float4(0.0, 0.0, 0.1, 0.1);
            half _glow;
            ...
            {
                v2f o = (v2f)0;
                o.uv0.xy = v.texcoord0.xy + v.texcoord0.zw * _UseCustomData1xy;
                ...
                o.customData1.z = v.customData1.x;
                //使用UI裁切
                #if ENABLE_UICLIP
            ...
            half4 frag(v2f i, float facing : VFACE) : COLOR
            {
                ...
                //使用UV运动          //----------------DOD
                #if FX_UVTRANSFORM
                UV = (float2((_Time.g * _U), (_Time.g * _V)) + i.uv0.xy);
                #endif
                ...
                /*_Texture_var = tex2D(_Texture, TRANSFORM_TEX(DistortUV, _Texture));*/

                /*finalColor.rgb = (_Texture_var.rgb)*(_TintColor.a*_Texture_var.a*i.vertexColor.a)*(_TintColor.rgb*_glow);*/
                #endif

                half4 _Texture_var = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, TRANSFORM_TEX(UV, _MainTex));

                half3 emissive = (_Texture_var.rgb * i.vertexColor.rgb * (_TintColor.rgb * _glow));
                half3 finalColor = emissive;
                half finalAlpha = 1.0;
                #if ENABLE_Alpha
                finalAlpha = lerp(1, (max(max(_UseAlpha.r * _Texture_var.r, _UseAlpha.g * _Texture_var.g),
                                          max(_UseAlpha.b * _Texture_var.b, _UseAlpha.a * _Texture_var.a))), 1) * i.
                                                                                                                  vertexColor.a * _TintColor.a;
                #endif
                ...
                //使用UV运动
                #if FX_UVTRANSFORM
                emissive = (_Texture_var.rgb * i.vertexColor.rgb * (_TintColor.rgb * _glow));
                finalColor = emissive;
                #endif
                ...
                finalAlpha *= (DissolveValue) * max(step(rimflag, 0.0001), RimDissolveMap_var);
                ...
                finalColor = finalColor * step(0, min(factor.x, factor.y));
                finalAlpha *= step(0, min(factor.x, factor.y));
                #endif
                ...
                half FresnelPower = pow(1.0 - max(0, dot(i.normalDir, viewDirection)), _FresnelStr);
                finalColor += (FresnelPower * lerp(0, finalColor, _FresnelStr));
                finalAlpha *= FresnelPower;
                #endif

                #if FX_MASK
                finalAlpha *= lerp(_Mask_var.r, _isLux, _isLux);
                ...
                //#endif
                finalAlpha = saturate(finalAlpha * _glow);
    ...
    CustomEditor "EffectMaterialGUI_Combine_AlphaBlend"
}