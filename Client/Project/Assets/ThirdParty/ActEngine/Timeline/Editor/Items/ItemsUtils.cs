...
namespace UnityEditor.ActTimeline
{
    static class ItemsUtils
    {
        ...
        static readonly Dictionary<TimelineClip, ClipItem> s_ActClipCache = new Dictionary<TimelineClip, ClipItem>();
        ...
        public static IEnumerable<ITimelineItem> GetActItemsExcept(this int row, IEnumerable<ITimelineItem> items)
        {
            IEnumerable<TimelineClipGUI> actClips = ActTimelineUtility.GetClipGUIList(row);
            var list = actClips.Select(clip => (ITimelineItem)new ClipItem(clip.clip)).ToList();

            list = list.OrderBy(x => x.start).ThenBy(x => x.end).ToList();

            return list.Except(items);
        }
