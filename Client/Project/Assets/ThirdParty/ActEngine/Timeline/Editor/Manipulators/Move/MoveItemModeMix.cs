...
namespace UnityEditor.ActTimeline
{
    class MoveItemModeMix : IMoveItemMode, IMoveItemDrawer
    {
        ...
        public void UpdateMove(IEnumerable<ItemsPerTrack> itemsGroups)
        {
            ...
            {
                TrackExtensions.ComputeBlendsFromOverlaps(items.clips.ToArray());
            }
        ...
        {
            var allClips = itemsGroups.SelectMany(i => i.clips);
        ...
        public bool ValidateMove(ItemsPerTrack itemsGroup)
        {
            ...
            if (TimelineSwitch.isUseNew)
            {
                var siblings = ItemsUtils.GetActItemsExcept(itemsGroup.row, items);
