...
namespace UnityEditor.ActTimeline
{
    ...
    static class ItemSelection
    {
        ...
        public static void RangeSelectItems(ITimelineItem lastItemToSelect)
        {
            ...
            if (TimelineSwitch.isUseNew)
            {
                var allItems = ActTimelineUtility.GetClipGUIList().Select(d => d.item).ToList();
        ...
        public static ISelectable HandleSingleSelection(Event evt)
        {
            ...
            {
                //Debug.Log("<color=#B0FF9C>选中失败</color>");
            }
    ...
    partial class SelectAndMoveItem : Manipulator
    {
        bool m_Dragged;
        ...
        protected override bool MouseDrag(Event evt, WindowState state)
        {
            ...
            if (!m_Dragged)
            {
                ...
                if (canStartMove)
                {
                    Debug.Log($"Start Move Selected {selectedItemsGUI.Count()}");
                    ...
                    m_MoveItemHandler.ActGrab(selectedItemsGUI, m_MouseDownPosition);
            ...
            if (m_Dragged)
            {
                ...
                if (TimelineSwitch.isUseNew)
                {
                    if (m_VerticalMovementDone && EditMode.AllowTrackSwitch())
                    {
                        var track = state.spacePartitioner.GetItemsAtPosition<IRowGUI>(evt.mousePosition).FirstOrDefault();
                        if (track == null)
                        {
                            return false;
                        }

                        var trackGUI = track as ActTimelineTrackGUI;

                        m_MoveItemHandler.UpdateRow(trackGUI.id);
                    }
