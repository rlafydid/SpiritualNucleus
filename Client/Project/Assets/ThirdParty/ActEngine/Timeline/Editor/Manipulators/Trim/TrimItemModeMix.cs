...
namespace UnityEditor.ActTimeline
{
    class TrimItemModeMix : ITrimItemMode, ITrimItemDrawer
    {
        ...
        public void OnBeforeTrim(ITrimmable item, TrimEdge trimDirection)
        {
            ...
            IOrderedEnumerable<ITrimmable> sortedItems = null;
            if (TimelineSwitch.isUseNew)
            {
                sortedItems = ItemsUtils.GetActItemsExcept(item.gui.row, new[] { item })
                .OfType<ITrimmable>()
                .OrderBy(c => c.start);
            }
            else
            {
                sortedItems = ItemsUtils.GetItemsExcept(item.parentTrack, new[] { item })
                   .OfType<ITrimmable>()
                   .OrderBy(c => c.start);
            }
