...
namespace UnityEditor.ActTimeline
{
    class SequenceHierarchy : ScriptableObject
    {
        ...
        public void Add(Act.ActAsset asset)
        {
        ...
        public void FromSequencePath(SequencePath path, bool forceRebuild)
        {
            ...
            if (TimelineSwitch.isNormalScrollScale)
            {
                //待处理
            ...
            Add_Internal(TimelineWindow.instance.actData);
            ...
            //if (ValidateSubElements(path.subElements, null, out buildingBlocks))
            //{
        ...
        void Add_Internal(Act.ActAsset asset)
        {
            Clear_Internal();

            var parent = m_Sequences.Count > 0 ? editSequence : null;
            m_Sequences.Add(new SequenceState(m_WindowState, asset, (SequenceState)parent));
        }
