...
namespace UnityEditor.ActTimeline
{
    ...
    class BreadcrumbDrawer
    {
        ...
        public static void DrawActAsset(float breadcrumbAreaWidth, List<BreadCrumbTitle> labels, Action<int> navigateToBreadcrumbIndex)
        {
            GUILayout.BeginHorizontal();
            {
                var labelWidth = (int)(breadcrumbAreaWidth / labels.Count);
        ...
        static Rect DrawActiveBreadcrumb(int width, BreadCrumbTitle label, GUIStyle style, GUIStyle backgroundStyle)
        {
            ...
            if (GUI.Button(rect, content, style))
            {
                ...
                UnityEngine.Object target = TimelineWindow.instance.actData;
                if (target != null)
                {
                    bool ping = true;
