...
namespace UnityEditor.ActTimeline
{
    static class ClipModifier
    {
        ...
        public static bool Delete(TimelineClip clip)
        {
            if (clip.GetClipType == ClipType.Clip)
            {
                var actClip = clip.GetTarget as Act.ActBaseClip;
                actClip.Destory();
                TimelineWindow.instance.actData.Clips.Remove(actClip);
            }
            else
            {
                var actEvent = clip.GetTarget as Act.ActBaseEvent;
                TimelineWindow.instance.actData.Events.Remove(actEvent);
            }
            Act.ActTimelineDebugger.RefreshData();
            EditorClipFactory.RefreshEditors();
            return true;
        }

        public static void CreateClip(int row, Type type)
        {
        ...
        public static void CreateEvent(int row, Type type)
        {
            var clips = ActTimelineUtility.GetEventGUIList(row);
            clips = clips.OrderBy(d => d.time).ToList();
            ...
            var actEvent = (Act.ActBaseEvent)Activator.CreateInstance(type);
            actEvent.TriggerTime = triggerTime;
            actEvent.Row = row;
            actEvent.CreateGUID();
            //clip.Asset = TimelineWindow.instance.actData;
            actEvent.AddToTimeline();
            Add(actEvent);
        }

        public static bool Add(Act.ActBaseClip clip)
        {
            TimelineWindow.instance.actData.Clips.Add(clip);
            Act.ActTimelineDebugger.RefreshData();
            return true;
        }

        public static bool Add(Act.ActBaseEvent actEvent)
        {
            TimelineWindow.instance.actData.Events.Add(actEvent);
            Act.ActTimelineDebugger.RefreshData();
            return true;
        }
