...
namespace UnityEditor.ActTimeline
{
    ...
    partial class TimelineWindow : TimelineEditorWindow, IWindowStateProvider
    {
        [Serializable]
        ...
        public List<TimelineTrackBaseGUI> allTracks
        {
            get
            {
                return new List<TimelineTrackBaseGUI>();
            }
        }
        ...
        public Act.ActAsset actData;

        public void Init(Act.ActAsset actData)
        {
            this.actData = actData;

            foreach (var clip in actData.Clips)
            {
                clip.AddToTimeline();
            }

            EditorUtility.SetDirty(this.actData);

            LoadModel();

            RefreshMaxTime();

            Act.Facade.Internals.Timeline.AddFrame = AddFrame;
            Act.Facade.Internals.Timeline.Reset = OnReset;
        }

        public void RefreshMaxTime()
        {
            float maxTime = actData.GetMaxTime();
            state.maxTime = maxTime;
            if (actData.LifeTime > maxTime)
                actData.LifeTime = maxTime;
        }
        ...
        void OnStateChange()
        {
            ...
            if (treeView != null && state.IsReadyAsset)
                treeView.Reload();
            RefreshMaxTime();
        }
        ...
        void DetectStateChanges()
        {
            if (state != null)
            {
                ...
                if (m_LastFrameHadSequence && state.IsEmptyAsset)
                {
                    ClearTimeline();
                }
                m_LastFrameHadSequence = state.IsReadyAsset;
        ...
        void InitializeGUIIfRequired()
        {
            ...
            InitializeTimeArea();
            if (treeView == null /*&& state.IsReadyAsset 试删除 */ )
            {
        ...
        {
            if (state == null || currentMode.mode != TimelineModes.Active || state.editSequence.director == null || state.IsEmptyAsset)
                return;
        ...
        Act.ActTimelineDebugger frameDebugger;
        bool isDebugging = false;
        public static void ShowWindow(Act.ActAsset asset)
        {
            Debug.Log($"打开时间轴编辑器 {asset.Clips.Count}");
            TimelineWindow window = GetWindow<TimelineWindow>();
            window.Focus();
            window.Init(asset);
            window.SetCurrentTimeline(asset);

            window.StartDebugger();
        }
        ...
        public void StartDebugger()
        {
            ...
            Act.Facade.Internals.Timeline.AddFrame = AddFrame;
            Act.Facade.Internals.Timeline.Reset = OnReset;
            Act.ActTimelineDebugger.CreateDebugger(actData);
            isDebugging = true;
        }
        ...
        {
            var assetDoubleClicked = EditorUtility.InstanceIDToObject(instanceID) as Act.ActAsset;
            if (assetDoubleClicked == null)
                return false;

            ShowWindow(assetDoubleClicked);
        ...
        //{
        //    bool disabled = state == null || state.IsEmptyAsset;
