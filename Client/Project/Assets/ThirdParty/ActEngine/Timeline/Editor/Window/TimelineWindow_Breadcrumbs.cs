...
namespace UnityEditor.ActTimeline
{
    partial class TimelineWindow
    {
        ...
        void ShowSequenceSelector()
        {
            ...
            //}
            var actList = Act.ActEditorUtility.GetActAssetsByModel(actData.Model);

            if (actList == null)
                sequenceMenu.AddDisabledItem(DirectorStyles.noAssetsInModel);
            else
            {
                foreach (var item in actList)
                {
                    sequenceMenu.AddItem(new GUIContent(item.name), item == actData, OnSequenceSelected, item);
                }
            }
