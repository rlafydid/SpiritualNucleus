...
namespace UnityEditor.ActTimeline
{
    partial class TimelineWindow
    {
        ...
        void DurationGUI(TimelineItemArea area, double duration)
        {
            ...
            if (state.GetDurationMode == TimelineAsset.DurationMode.BasedOnClips && duration <= 0.0f)
                return;
        ...
        void DrawDuration(bool drawhead, bool drawline, double duration)
        {
            if (state.TimeIsInRange((float)duration))
            {
                ...
                bool canMoveHead = !EditorApplication.isPlaying && state.GetDurationMode == TimelineAsset.DurationMode.FixedLength;
            ...
            if (state.IsReadyAsset && drawhead)
            {
        ...
        {
            if (state.GetDurationMode == TimelineAsset.DurationMode.FixedLength && !state.editSequence.isReadOnly)
            {
