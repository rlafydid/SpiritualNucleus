...
namespace UnityEditor.ActTimeline
{
    partial class TimelineWindow
    {
        ...
        internal TimelineMode currentMode
        {
            ...
            {
                return s_ActiveMode;
                if (state == null || state.IsEmptyAsset)
        ...
        public void DrawLastMouse()
        {
            var x = PickerUtils.lastMousePos.x;
            Rect rect = new Rect(x, PickerUtils.lastMousePos.y, 200, 30);
            DrawDebug.Draw(rect, $"pos:{x} {state.PixelToTime(x)}");
        }
        ...
        void DrawHeaderBackgroundBottomFiller()
        {
            ...
            if (state.IsReadyAsset && !state.IsEditingAnEmptyTimeline())
            {
        ...
        void DrawToolbar()
        {
            ...
            using (new GUILayout.VerticalScope())
            {
                using (new GUILayout.HorizontalScope(EditorStyles.toolbar))
                {
                    ...
                    {
                        DrawTransportToolbar(); //帧控制按钮
                    }


                    DrawSequenceSelector(); //向下箭头
                    DrawBreadcrumbs(); //Timeline文件名和图标
        ...
        void SubTimelineRangeGUI()
        {
            ...
            if (fullRect.xMax > area.y)
            {
                ...
                after.yMax = state.timeAreaRect.y + state.timeAreaRect.height + 0.0f;
                EditorGUI.DrawRect(after, subTimelineOverlayColor);
            }
        }
        ...
        void DrawOptions()
        {
            //if (currentMode.headerState.options == TimelineModeGUIState.Hidden || state.IsEmptyAsset)
            //    return;
