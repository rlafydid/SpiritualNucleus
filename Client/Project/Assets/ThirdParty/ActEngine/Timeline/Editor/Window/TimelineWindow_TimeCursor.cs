...
namespace UnityEditor.ActTimeline
{
    partial class TimelineWindow
    {
        ...
        bool CanDrawTimeCursor(TimelineItemArea area)
        {
            ...
            if (treeView == null || state.IsEmptyAsset || (state.IsReadyAsset && state.IsEditingAnEmptyTimeline()))
                return false;
        ...
        {
            if (state.previewMode && isDebugging)
            {
                Act.ActTimelineDebugger.Simulate((float)state.editSequence.time);
            }
        }
        ...
        void AddFrame()
        {
            ...
            if (state.referenceSequence.time >= state.maxTime)
            {
                state.Pause();
            }
        }
