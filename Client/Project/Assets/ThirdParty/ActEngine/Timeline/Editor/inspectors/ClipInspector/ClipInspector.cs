    ...
    class ClipInspector : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            using (new EditorGUI.DisabledScope(false))
            {
                ...
                foreach (var item in m_SelectionCache)
                {
                    var actEditor = item.editorClip.actEditor;
                    string displayName = Act.ActUtility.GetDisplayName(item.editorClip.clip.GetTarget.GetType());
                    using (new EditorGUILayout.HorizontalScope())
                    {
                        bool isActive = EditorGUILayout.BeginFoldoutHeaderGroup(actEditor.IsActive, displayName, null, (pos) =>
                        {

                        });
                        if (actEditor.IsActive != isActive)
                        {
                            actEditor.SetActive(isActive);
                        }
                        EditorGUILayout.EndFoldoutHeaderGroup();
                    }

                    if (actEditor.IsActive)
                        actEditor.OnInspectorGUI();
