...
namespace DamageNumbersPro
{
    public abstract class DamageNumber : MonoBehaviour
    {
        ...
        //Collision & Push Immunity:
        float collisionImmuneTime;
        ...
        IEnumerator UpdateCoroutine()
        {
            ...
            {
                //Delta:
                lastUpdateTime = unscaledTime ? Time.unscaledTime : Time.time;
        ...
        public void PrewarmPool()
        {
            if (enablePooling)
            {
                ...
                if(amount > poolSize * 0.5f)
                {
                    ...
                    {
                        DamageNumber dn = Spawn(new Vector3(-9999999, -9999999, -9999999));
        ...
        protected virtual void SetTextString(string fullString)
        {
            ...
            for (int c = 0; c < textMeshPro.transform.childCount; c++)
            {
                ...
                if (subMeshRenderer != null)
                {
                    ...
                    Mesh newSubMesh = subMeshFilter.sharedMesh;
        ...
        void HandleVelocity(float deltaTime)
        {
            ...
            position += (up * currentVelocity.y + right * currentVelocity.x) * deltaTime;
        }
        ...
        Vector3 ApplyShake(Vector3 vector, ShakeSettings shakeSettings, float time)
        {
            ...
            {
                vector += up * currentFactor * shakeSettings.offset.y;
            }

            if (shakeSettings.offset.x != 0)
            {
                vector += right * currentFactor * shakeSettings.offset.x;
            }
        ...
        void TryCollision(Vector3 sourcePosition)
        {
            if (enableCollision)
            {
                collisionImmuneTime = Time.unscaledTime;
                ...
                foreach (DamageNumber otherNumber in spamGroupDictionary[spamGroup])
                {
                    if (otherNumber.enableCollision && otherNumber != this)
                    {
                        ...
                        if (distance < radius)
                        {
                            ...
                            if(otherNumber.collisionImmuneTime != Time.unscaledTime)
                            {
        ...
        void TryPush(Vector3 sourcePosition)
        {
            if (enablePush)
            {
                ...
                float lowestHeight = myTargetPosition.y + 1000f;
                foreach (DamageNumber otherNumber in spamGroupDictionary[spamGroup])
                {
                    if (otherNumber.enablePush && otherNumber.collisionImmuneTime != Time.unscaledTime)
                    {
                        Vector3 targetPosition = otherNumber.GetTargetPosition();
                        if (targetPosition.y < lowestHeight && Vector3.Distance(myTargetPosition, targetPosition) < radius)
                        {
                            lowestHeight = targetPosition.y;
                ...
                {
                    float heightDifference = lowestHeight - myTargetPosition.y;

                    bestPushTarget.remainingOffset.y += Mathf.Max((pushSettings.pushOffset * GetPositionFactor() - heightDifference), 0);
        ...
        void UpdateScaleAnd3D()
        {
            ...
            {
                //Face Camera:
                Vector3 newEulerAngles = targetCamera.eulerAngles;
