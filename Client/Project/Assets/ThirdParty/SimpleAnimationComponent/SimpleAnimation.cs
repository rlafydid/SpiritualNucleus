...
public partial class SimpleAnimation : MonoBehaviour
{
    ...
    public State getPlayingState
    {
        get
        {
            if (_playingState == null || _playingState.name != m_Playable.CurState)
            {
                _playingState = GetState(m_Playable.CurState);
            }
    ...
    public void CrossFade(string stateName, float fadeLength, bool returnToDefaultState = false)
    {
        //Debug.Log($"Play {stateName}");
        this.returnToDefaultState = returnToDefaultState;
        m_Animator.enabled = true;
        Kick();
    ...
    /// <param name="returnToDefaultState"> 播放结束后自动回到默认状态 </param>
    /// <returns></returns>
    public bool Play(string stateName, bool returnToDefaultState = false)
    {
        this.returnToDefaultState = returnToDefaultState;
        m_Animator.enabled = true;
        Kick();
